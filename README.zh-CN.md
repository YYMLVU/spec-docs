...
> ✅ **Spec Docs 2.0.0** 覆盖全部 8 种模式、架构治理、hook 脚手架，以及源码形态和安装后 package 形态的本地完备性检查。

## 🚀 快速开始

### 平台支持

- **主要支持：Claude Code** skill 安装与使用。
- **手动或实验性集成：** Cursor、GitHub Copilot、OpenAI agents 和其他 agent 系统只有在该环境的安装、调用和验证行为已经确认时，才应把本 skill 文件作为 prompt/documentation package 使用。
...
| 🏗️ 架构治理 | 记录放置规则、当前架构、目标架构和架构 review。 |
...
| 🪝 可选 Hook 层 | `hooks/` | agent hook 集成的骨架/提醒占位。Claude hook 脚手架覆盖 `SessionStart`、`PreToolUse`（Edit/Write/MultiEdit 加 Bash）、`PostToolUse` 和 `Stop`；Cursor hook 脚手架覆盖 `sessionStart` 和 `stop`。Hook 不替代规则；它只检测事件并指向所需的模式或引用。Hook 不得自动修改代码、ADR 或架构规则。当前 hooks 仅为集成脚手架，并非生产级强制层，且带有 session/change-unit reminder deduplication。 |
...
`init` 会根据项目 profile 调整输出范围：

- **Empty Project** 只接收最小原则种子；不会创建假的 implementation specs 或 inventory indexes。
- **Minimal Existing Project** 接收 README、constitution、inventory 和 project overview；默认不创建 architecture docs、ADRs、rebuild docs 或 child spec folders。
- **Standard Existing Project** 只为已观察到的实现区域创建 grounded child specs。
- **Large Project / Phased Init** 可以使用 `PARTIAL INIT`；只有 included-scope coverage 通过验证后，才能声明 final init completion。

已有 `docs/spec-docs/` 工作区仍然有效。Adaptive profiles 指导后续 `init` 行为；它们不要求删除已有 specs 或 architecture docs。
...
| 🏗️ `architecture/` | 当前架构规则、放置规则、调试规则（debugging rules）、目标架构，以及 rebuild 期间的 adoption plans。 |
...
| 🔄 `update` | 代码已经变更 | 使用 Code-to-Spec、Task-to-Spec、Symbol-to-Spec 映射同步受影响 specs 和 `inventory.md`。Routine changes 按 Level 0-4 impact routing 处理：Level 0（comment-only 或 metadata-only）可仅说明 no-update reason；Level 1（单 spec 行为变更）只更新 mapped spec；Level 2（局部变更）使用 targeted light check；Level 3（广泛变更）需要 full verify；Level 4（architecture-risk 变更）需要升级处理。 |
...
| 🩹 `repair` | specs 过期或不一致 | 将文档和项目规则重新对齐当前代码；发现疑似代码问题时只报告，未经用户明确请求不主动修改代码。 |
...
| 🏗️ `rebuild` | 启动目标架构迁移 | 定义目标架构、adoption plan 和 rebuild 状态，并在迁移期间保持 specs 对齐。 |
| 📦 `adopt` | 完成 rebuild 迁移，或为一个证据明确的区域引入 scoped governance | 将 target architecture 合并到 current architecture，更新 ADR evidence，标记 rebuild completed，并归档 rebuild 文档。也支持 scoped adopt，用于为一个证据充分的区域引入治理，而不要求完整 rebuild。 |
...
对于已有实现的项目，`init` completion 取决于 adaptive profile：

- **Minimal Existing Project：** minimal workspace 通过 inventory 和 project overview 覆盖 included implementation files 时，`init` 完成。
- **Standard Existing Project：** Code-to-Spec Index 覆盖所有 included implementation-relevant files 时，`init` 完成。
- **Large Project / Phased Init：** 可以使用 `PARTIAL INIT`；这不是 final completion，final completion 仍要求 included-scope coverage verification。
...
协议会要求后续 AI 在 implementation changes 前阅读相关 specs，在这些 changes 之后分类 implementation impact，再在声明完成前完成 impact-appropriate spec action：Level 0 说明 no-update reason，Level 1 更新 affected spec，Level 2 执行 targeted light check，Level 3 执行 full verify，Level 4 进行 architecture-risk escalation 并在声明 architecture-current 前执行 full verify。
...
对于架构规则，依次参考 `architecture/current-architecture.md`、`architecture/placement-rules.md`、Accepted ADRs，然后是 specs。决策原因以 ADRs 为第一事实源。
...
在上表中，`$spec-docs` 表示 “the spec-docs skill”；可以按你的 agent 习惯改写请求。`verify`、`place`、`repair` 和 `adopt` 在不需要 full workspace pass 时支持 scoped subpaths。

## ✅ 发布与完备性说明

> 📦 **版本 `2.0.0`** 已对源码形态和安装后 package 形态进行本地完备性覆盖。
>
> 当前本地套件覆盖二十八个场景，并覆盖全部八种模式：`init`、`update`、`verify`、`repair`、`place`、`rebuild`、`adopt` 和 `diagnose`。
...
- ✅ 空项目与已有项目的 `init`
- ✅ 架构放置与边界 review
- ✅ scoped placement、scoped repair、repair escalation、rebuild recommendation 和 adopt scope distinction
- ✅ Level 0-4 impact-aware routing，避免 Level 0-2 被过度触发 full verify
- ✅ targeted-check、layered-check 和 full-verify scope separation
...
> ⚠️ **已知限制：** `greenfield` Adoption Mode 语义仍需要产品决策；live protocol-block synchronization 不属于当前本地 static/script/simulation hook 套件范围。
...
3. 更新 `AGENTS.md` 或 `CLAUDE.md` 中位于 `<!-- SPEC-DOCS-PROTOCOL:BEGIN -->` 和 `<!-- SPEC-DOCS-PROTOCOL:END -->` 之间的 marked protocol block；如果该 block 尚不存在，运行 `spec-docs init` 或 `spec-docs repair` 插入它，且不要改写无关项目指令。
4. 分类 migration impact 并完成 impact-appropriate spec action；只有 impact level 或显式 currentness claim 要求时才运行 full `spec-docs verify`。
...
│       │   ├── hooks.json
│       │   ├── hooks-cursor.json
│       │   ├── run-hook.cmd      # Windows wrapper；POSIX shell 直接运行 scripts/
│       │   └── 📁 scripts/
│       │       ├── session-start
│       │       ├── pre-edit-guard
│       │       ├── pre-bash-guard
...
│   └── openai.yaml
├── 📁 test-runs/
│   └── 📁 spec-docs-completeness/   # 本地完备性套件：28 个场景、全部模式、source + installed 检查
├── package.json
├── package-lock.json
├── README.md
├── README.zh-CN.md
├── INSTALL-FOR-AI.md
├── 📁 docs/                # 内部设计说明，不随 npm package 发布
├── head.png
├── LICENSE
└── .gitignore
```
