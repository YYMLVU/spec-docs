...
| 🏗️ `architecture/` | 当前架构规则、放置规则、目标架构，以及 rebuild 期间的 adoption plans。 |
...
| 🔄 `update` | 代码已经变更 | 使用 Code-to-Spec、Task-to-Spec、Symbol-to-Spec 映射同步受影响 specs 和 `inventory.md`。Routine changes 会按 impact 分类：comment-only 或 metadata-only 变更可说明 no-update reason；单 spec 行为变更只更新 mapped spec；局部变更使用 targeted light check；广泛或 architecture-risk 变更仍需 full verification 或升级处理。 |
...
协议会要求后续 AI 在改 implementation-relevant files 前阅读相关 specs，改完后先分类 implementation impact，再在声明完成前完成 impact-appropriate spec action：Level 0 说明 no-update reason，Level 1 更新 affected spec，Level 2 执行 targeted light check，Level 3 执行 full verify，Level 4 报告 architecture risk 并在声明 architecture-current 前执行 full verify。
