...
| Level 4 update with architecture-current claim | `full-verify` | mechanical, mapping, semantic for affected facts, architecture, freshness | Phase 4 architecture subpaths may scope the receiving workflow, but architecture-current claims still require full verify. Ordinary update must not legalize architecture drift. |
